...
/** 불법 가능성 검토 단계 (확정 아님) */
export type BannerReviewTier = "urgent" | "priority" | "normal" | "observe";

/** 공무원 처리 상태 */
export type BannerCandidateStatus = "pending" | "reviewing" | "held" | "resolved";

export interface IllegalScoreBreakdown {
  unpermitted: number;
  improperLocation: number;
  expiredPeriod: number;
  trafficObstruction: number;
  repeatComplaint: number;
}
...
export interface BannerCandidate {
  ...
  reviewTier: BannerReviewTier;
  status: BannerCandidateStatus;
  reasons: string[];
}
